...
n_combination="4"
train_batch_size="2"
root_dataset_dir="/Users/bing/Documents/mlProfiler/tests/mps/multiinstance/dataset/comb${n_combination}/batch2-8/MPS100/unseen_partition_batch"
if [ "$modeltype" == "hotcloud" ]
then
    full_data_path="/Users/bing/Documents/mlProfiler/tests/mps/multiinstance/dataset/0906hotcloud_total_labels_comb4_batch2-8_targetMPS100.csv"
else
    #KACE
    #full_data_path="/Users/bing/Documents/mlProfiler/tests/mps/multiinstance/dataset/total_labels_batch2-8_targetMPS100.csv"
    full_data_path="/Users/bing/Documents/mlProfiler/tests/mps/multiinstance/dataset/0906_total_labels_comb4_batch2-8_targetMPS100.csv"
fi
...
unseen_workloads=('whisper-large-v2_batch8-inf'
...
then
    ...
    do
        echo "seed $seed"
        dataset_dir="$root_dataset_dir/rand${seed}/$modeltype/trainbatch$train_batch_size"
        outdir_rand="$outdir/unseen_partition/rand${seed}/$modeltype/trainbatch$train_batch_size"
        mkdir -p $dataset_dir
        mkdir -p $outdir_rand
        ...
        then
            ...
            python /Users/bing/Documents/mlProfiler/output/dataset/MPS100/partition_exp/partition_data_by_batch.py \
            $full_data_path $dataset_dir $n_combination $train_batch_size $test_file $train_file
...
#Predict throughput for different workloads
    ...
    do
        ...
        echo "seed $seed"
        dataset_dir="$root_dataset_dir/rand${seed}/$modeltype/trainbatch$train_batch_size"
        model_dir="$outdir/unseen_partition/rand${seed}/$modeltype/trainbatch$train_batch_size"
        outdir_rand="$outdir/unseen_partition/rand${seed}/$modeltype/trainbatch$train_batch_size"
        #outdir_pred="$outdir_rand/predictthroughput"
        outdir_pred="$outdir_rand/predicts_by_batch"
        mkdir -p $outdir_pred
...
#Predict throughput for different workloads
    ...
    #partition testing test

    echo "start partitioning predict set for different workloads"
    for workload in "${unseen_workloads[@]}";
        do
        for seed in "${seeds[@]}"
        do
            echo "seed $seed"
            all_testing_data_dir="$root_dataset_dir/rand${seed}/$modeltype/trainbatch$train_batch_size"
            dataset_dir="$root_dataset_dir/rand${seed}/$modeltype/trainbatch$train_batch_size/$workload"
            outdir_rand="$outdir/unseen_partition/rand${seed}/$modeltype/trainbatch$train_batch_size/$ispredict/$workload"
            mkdir -p $dataset_dir
            mkdir -p $outdir_rand
            #if data is not generated, generate data
            if [ ! -f "$all_testing_data_dir/$train_file" ] ||  [ ! -f "$all_testing_data_dir/$test_file" ] 
            then
            #error and exit
                echo "training and all test data in $all_testing_data_dir not found"
                exit 1
            fi

            if [ ! -f "$dataset_dir/$test_file" ]
            then 
                echo "generate data for $dataset_dir..."
                echo ""
                chmod +x $dataset_dir
                #echo "$full_data_path $workload $dataset_dir $n_combination"
                python /Users/bing/Documents/mlProfiler/output/dataset/MPS100/partition_exp/partition_data_by_workloadname.py \
                $all_testing_data_dir/$test_file $workload $dataset_dir $n_combination split_test_only
            fi
            #python main.py  --test_file $dataset_dir/$test_file \
            #--train_file  $dataset_dir/$train_file \
            #-rd $seed --targetMPS $targetMPS \
    ...
    do
        ...
        do
            ...
            echo "seed $seed"
            training_data_dir="$root_dataset_dir/rand${seed}/$modeltype/trainbatch$train_batch_size"
            dataset_dir="$root_dataset_dir/rand${seed}/$modeltype/trainbatch$train_batch_size/$workload"
            model_dir="$outdir/unseen_partition/rand${seed}/$modeltype/trainbatch$train_batch_size"
            outdir_rand="$outdir/unseen_partition/rand${seed}/$modeltype/trainbatch$train_batch_size/predicts_by_workloads/$workload"
            #outdir_pred="$outdir_rand/predictthroughput"
            ...
            python ../../../main.py  --test_file $dataset_dir/$test_file \
            --train_file  $training_data_dir/$train_file \
            --HPworkload "" --targetMPS 100 \
            --model $model_file -corr $correlation \
            -rd $seed --output_dir $outdir_pred \
            --rulebase --modeltype $modeltype \
