...
#TRAIN
#create LS_MODELS as task:model:epoch:batch_size by creating entry for each task with batch size 4,32,64

#LS_MODELS=("recommend:bert-base-cased:2000000:8" "imgclassification:mobilenet:200000:64" "imgclassification:vit_h_14:200000:8" "recommend:albert/albert-base-v2:2000000:8"  "imgclassification:resnet-50:200000:32")
...
LS_batch_sizes=(4)
BE_batch_sizes=(2 8 16)
#enter this loop if LS_MODE is train
if [ "$LS_MODE" == "train" ]; then
    ...
    for task_model in "${train_candidates[@]}"; do
        ...
        for batch_size in "${LS_batch_sizes[@]}"; do
...
if [ "$BE_MODE" == "train" ]; then
    ...
    for task_model in "${train_candidates[@]}"; do
        # Split the string into task and model using IFS (Internal Field Separator)
        IFS=":" read -r task model <<< "$task_model"
        for batch_size in "${BE_batch_sizes[@]}"; do
            # Append the task:model:epoch:batch_size to the LS_MODELS array
...
#inference
#TASKS_MODELS=("speech-recognition:openai/whisper-large-v2:0:1" "imgclassification:google/mobilenet_v2_1.0_224:0:64" "recommend:bert-base-cased:0:8" "imgclassification:google/vit-base-patch16-224:0:1" "imgclassification:microsoft/resnet-50:0:32"  "speech-recognition:facebook/wav2vec2-base-960h:0:1" )
#LS single inference test
#TASKS_MODELS=("imgclassification:google/mobilenet_v2_1.0_224:0:64")

#BE TRAIN
#TASKS_MODELS=("imgclassification:vit_h_14:2:8" "recommend:albert/albert-base-v2:2:8" "recommend:bert-base-cased:2:8" "imgclassification:mobilenet:2:64" "imgclassification:resnet-50:2:32")
#BE single train test
#TASKS_MODELS=("recommend:albert/albert-base-v2:1:8")

#BE_EPOCHS=(7 7) 

LS_PERCENTAGES=(100 50 10 20 30 40 60 70 80 90)
...
#LS_PERCENTAGES=(90 100 50)
#BE_PERCENTAGES=(10 100 50)
RUNS=1
            ...
            for TASK_MODEL in "${TASKS_MODELS[@]}"; do
                ...
                if [ $LS_PERCENTAGE -eq 0 ]; then
                    ...
                    if [ "$LS_MODE" == "train" ]; then
                        ...
                        /bin/sh -c "cd /root/mlprofiler/workloads/training; python ${LS_TASK}-train.py --model_name ${LS_MODEL} --n_epoch ${LS_EPOCH}\
                        ...
                        --batch_size ${LS_BATCH} --log_dir ../../tests/mps/$CURR_LOG_DIR --profile_nstep 10000"  \
                        > $CURR_LOG_DIR/LS_${LS_TASK}_MPS${LS_PERCENTAGE}.log 2>&1 & ls_pid=$!
                    fi
                fi
                ...
                if [ "$BE_MODE" == "train" ]; then
                    ...
                    --model_name ${MODEL} --batch_size ${BE_BATCH} --profile_nstep 200" \
